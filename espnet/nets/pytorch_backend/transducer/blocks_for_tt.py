...
from espnet.nets.pytorch_backend.transducer.transformer_decoder_layer_for_tt import (
...
from espnet.nets.pytorch_backend.transformer.encoder_layer_for_tt import EncoderLayer
from espnet.nets.pytorch_backend.transformer.embedding import (
    PositionalEncoding,  # noqa: H301
    ScaledPositionalEncoding,  # noqa: H301
    RelPositionalEncoding,  # noqa: H301
)
from espnet.nets.pytorch_backend.transformer.positionwise_feed_forward_for_tt import (
...
def check_and_prepare(net_part, blocks_arch, input_layer):
    ...
    for i in range(len(blocks_arch)):
        ...
        if block_type == "transformer":
            if not {"d_hidden", "d_ff_1", "d_ff_2", "heads", "head_dim", "normalize_before", "concat_after"}.issubset(blocks_arch[i]):
                raise ValueError(
                    "Block "
                    + str(i + 1)
                    + "in "
                    ...
                    "'d_hidden': int, 'd_ff_1': int, 'd_ff_2': int, 'heads': int, 'head_dim': int, 'normalize_before': bool, 'concat_after': bool, [...]}"
                )
...
def build_transformer_block(
    ...
    d_ff_1 = block_arch["d_ff_1"]
    d_ff_2 = block_arch["d_ff_2"]
    heads = block_arch["heads"]
    head_dim = block_arch["head_dim"]
    normalize_before = block_arch["normalize_before"]
    concat_after = block_arch["concat_after"]
    ...
    if pw_layer_type == "linear":
        ...
        pw_layer_args = (d_hidden, d_ff_1, d_ff_2, pos_dropout_rate, pw_activation)
    else:
    ...
    return lambda: transformer_layer_class(
        d_hidden,
        self_attn_class(heads, head_dim, att_dropout_rate),
