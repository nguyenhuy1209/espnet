...
from espnet.nets.pytorch_backend.transducer.arguments_for_tt import (
...
from espnet.nets.pytorch_backend.transducer.custom_decoder_for_tt import CustomDecoder
from espnet.nets.pytorch_backend.transducer.custom_encoder_for_tt import CustomEncoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1, blank_id=0, training=True):
        ...
        if "custom" in args.dtype:
            ...
            self.decoder = CustomDecoder(
                ...
                self_attn_type=args.custom_dec_self_attn_type,
                positional_encoding_type=args.custom_dec_positional_encoding_type,
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if "custom" in self.etype:
            ...
            # Modify for attention masking for input features
            ####################################################
            src_mask = make_non_pad_mask(ilens.tolist()).to(xs_pad.device).unsqueeze(-2)
            # streaming_mask = target_mask(xs_pad[:,:,0], self.blank_id)
            # src_mask = mask1 & mask2
            # print(src_mask.size())
            # if not torch.all(mask1).data:
            #   print(src_mask.size())
            #   return
            # return
            ####################################################
    ...
    def encode_custom(self, x):
        ...
        #############################################
        # mask = target_mask(x[:,:,0], self.blank_id)
