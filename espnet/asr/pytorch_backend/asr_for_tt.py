...
from espnet.nets.beam_search_transducer_for_tt import BeamSearchTransducer
from espnet.nets.pytorch_backend.e2e_asr import pad_list
import espnet.nets.pytorch_backend.lm.default as lm_pytorch
from espnet.nets.pytorch_backend.streaming.segment_for_tt import SegmentStreamingE2E
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    se2e = SegmentStreamingE2E(e2e=model, recog_args=args, beam_search=beam_search_transducer, rnnlm=rnnlm)
                    r = np.prod(model.subsample)
                    for i in range(0, feat.shape[0], r):
                        hyps = se2e.accept_input_for_tt(feat[i : i + r])
                        # print(hyps)
                        if hyps is not None:
                            text = "".join(
                                [
